...
### oplist file read 
wop_list=[]
wop_list_fname = "wop_list"
wop_list_fo = open(wop_list_fname)
wop_raw_list = wop_list_fo.readlines() 
for wop in wop_raw_list:
  wop_list.append(wop.rstrip())
wop_list_fo.close
#wop=["_write", "_setattrs", "_omap_setheader", "_omap_setkeys"]

count_per_op = [0 for i in range(len(wop_list))]
obj_tbl = {'obj' : count_per_op}
#obj_tbl = {'obj' : [0, 0, 0, 0]}
...
for i in range(len(bench)):
	...
	ofname = bench[i] + "/" + bench[i] + ".obj_cnt_all"

	for j in range(len(wop_list)):
		op =wop_list[j] 
		ifname = bench[i] + "/" + bench[i] + ".obj_cnt." + op
		print ifname
...
#				count[j] += long(cnt)  
				...
				obj_tbl[obj] = count
	### oplist file read 
	objfname = bench[i] + "/" + bench[i] + ".obj_list"
